...
begin
  ...
  if not found then
    return jsonb_build_object(
      'exists', false, 'now', v_now, 'elapsed_seconds', 0, 'elapsed_hours', 0,
      'time_band', 'new', 'intimacy_level', 'initial', 'intimacy_points', 0,
      'emotion_state', jsonb_build_object('primary','neutral','intensity',0),
      'action_state', 'NORMAL', 'last_user_message_at', null,
      'last_misaki_message_at', null, 'last_interaction_at', null,
      'silence_started_at', null, 'state_updated_at', null
    );
  end if;

  if v_state.last_interaction_at is not null then
    v_elapsed_seconds := greatest(
      floor(extract(epoch from (v_now - v_state.last_interaction_at)))::bigint, 0
    );
  ...
  return jsonb_build_object(
    'exists', true, 'now', v_now,
    'elapsed_seconds', v_elapsed_seconds, 'elapsed_hours', v_elapsed_hours,
    'time_band', v_band, 'intimacy_level', v_state.intimacy_level,
    'intimacy_points', v_state.intimacy_points, 'emotion_state', v_state.emotion_state,
    'action_state', v_state.action_state, 'last_user_message_at', v_state.last_user_message_at,
    ...
    'state_updated_at', v_state.state_updated_at
  );
end;
$$;
