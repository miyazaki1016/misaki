...
begin
  ...
  if v_state.last_interaction_at is not null then
    v_elapsed_seconds := greatest(floor(extract(epoch from (v_now - v_state.last_interaction_at)))::bigint, 0);
    v_elapsed_hours := round((v_elapsed_seconds::numeric / 3600), 2);

    v_band := case
      when v_elapsed_seconds < 6 * 3600 then 'recent'
      when v_elapsed_seconds < 24 * 3600 then 'same_day'
      when v_elapsed_seconds < 3 * 24 * 3600 then 'one_to_three_days'
      when v_elapsed_seconds < 7 * 24 * 3600 then 'three_to_seven_days'
...
grant execute on function public.get_relationship_time_context() to authenticated;
