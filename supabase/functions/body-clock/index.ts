...
async function processUser(
  ...
  const { data: finished, error: finishError } = await supabase.rpc("finish_misaki_body_clock_delivery", {
    ...
    p_photo_context: { currentTime, relationshipPoints: Math.max(0, Number(row.relationship_points)||0), selectorVersion: 1, source: "body_clock" },
    p_delay_minutes: Math.floor(randomDelayMs()/60000),
  });
  if (finishError) throw finishError;
  const push = row.notifications_enabled
    ? await relayPush(finished.deliveryId)
    : { sent: 0, removed: 0, skipped: "notifications_disabled" };

  return {
    userId: row.user_id,
    deliveryId: finished.deliveryId,
    photoId: selectedPhoto?.id ?? null,
    nextPushAt: finished.nextPushAt,
    pushesToday: finished.pushesToday,
    push,
  };
}
...
export async function handler(request: Request) {
  ...
  if (!secret || secret.length !== 64) return Response.json({error:"unauthorized_header",version:3},{status:401});
  ...
  if (authorized !== true) return Response.json({error:"unauthorized_secret",version:3},{status:401});
  const apiKey = Deno.env.get("GEMINI_API_KEY");
  let relayReady = false;
  let relayError: string | null = null;
  try { relayReady = (await relayPush("health")).ok === true; }
  catch (error) { relayError = error instanceof Error ? error.message : "relay_unavailable"; }
  const ready = {gemini:!!apiKey,pushRelay:relayReady};
  if (new URL(request.url).searchParams.get("mode")==="health") return Response.json({ok:Object.values(ready).every(Boolean),ready,relayError,version:"body-clock-relay-v1"});
  ...
  const results: any[] = [];
  // At most four claimed users, processed concurrently with network deadlines.
  let cursor=0;
  const worker=async()=>{
    while(cursor<rows.length) {
      const row=rows[cursor++];
      try {
        const result = await processUser(supabase,apiKey!,row);
        results.push({ok: !("failed" in result.push) || result.push.failed === 0, ...result});
      }
      catch(error) {
        console.error("BODY_CLOCK_USER_FAILED",row.user_id);
        results.push({ok:false,userId:row.user_id,error:error instanceof Error ? error.message : "delivery_failed"});
      }
    }
  };
  await Promise.all([worker(),worker(),worker(),worker()]);
  return Response.json({ok:results.every(r=>r.ok),checkedAt:new Date().toISOString(),claimed:rows.length,results},
