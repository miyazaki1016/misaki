...
if(input.action==="SULK"||input.emotion==="hurt"){
 if(input.action==="RECONNECT"&&intensity>=20&&!contactedRecently)return{shouldSend:true,strength:Math.min(100,45+Math.round(intensity*.35)),desire:"reconnect",reason:"repair_wants_contact"};
...
if(input.photoOpportunity===true&&!contactedRecently&&input.pushesToday===0&&input.intimacyLevel>=2&&input.emotion==="affectionate"&&intensity>=72&&(input.action==="TEASE"||input.action==="CHASE"))return{shouldSend:true,strength:Math.min(90,45+Math.round(intensity*.4)),desire:"share_photo",reason:"strong_affection_wants_to_show_something"};
if(!contactedRecently&&input.action==="TEASE"&&input.emotion==="affectionate"&&intensity>=50)return{shouldSend:true,strength:Math.min(100,40+Math.round(intensity*.35)),desire:"be_playful",reason:"affection_wants_playful_contact"};
if(!contactedRecently&&input.action==="CHASE"&&intensity>=35)return{shouldSend:true,strength:Math.min(100,45+Math.round(intensity*.4)),desire:input.emotion==="affectionate"?"be_close":"talk",reason:"active_feeling_wants_contact"};
if(!contactedRecently&&input.emotion==="affectionate"&&intensity>=65&&input.intimacyLevel>=2&&input.pushesToday===0)return{shouldSend:true,strength:Math.min(85,35+Math.round(intensity*.35)),desire:"be_close",reason:"strong_grounded_affection_wants_contact"};
if(!contactedRecently&&input.emotion==="happy"&&intensity>=70&&input.intimacyLevel>=2&&input.pushesToday===0)return{shouldSend:true,strength:Math.min(75,30+Math.round(intensity*.3)),desire:"talk",reason:"shared_happiness_wants_contact"};
