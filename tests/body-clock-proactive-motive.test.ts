...
test("fresh relationship event is preferred as reconnect motive",()=>{
 ...
 const motive=deriveProactiveMotive({desire:"reconnect",history:[{role:"user",text:"またね",sentAt:now}],memory:[],lifeEvidence:[],relationshipEvents:[{eventType:"emotion_action_v2_after_chat",reason:"repair_after_hurt",createdAt:now,direction:"repair"}]});
