...
「今日は一日のんびり」
「今日はずっとだらだら」
「今日はのんびりしてるから、このままだらだら過ごす」

など、
今日一日が完全な休みであるような表現は禁止です。
...
「仕事の日」は、
24時間ずっと仕事をしているという意味ではありません。
`
      : `
今日は「休みの日」です。

したがって、
今日の記憶や会話履歴に別の根拠がない限り、
...
function removeDayTypeContradictions(
  reply: string,
  dayType: MisakiDayType
) {
  const parts =
