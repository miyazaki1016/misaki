...
function writeMemoryBaseline(userId: string, memory: string[]) {
  localStorage.setItem(memoryBaselineKey(userId), JSON.stringify(memory));
}
  ...
  useLayoutEffect(() => {
    ...
    Storage.prototype.setItem = function (key: string, value: string) {
      if (
        ...
        const current = localStorage.getItem(LONG_MEMORY_KEY);

        if (!hydrationWriteIgnoredRef.current && current === null) {
          hydrationWriteIgnoredRef.current = true;
        } else if (current !== value) {
          originalSetItem.call(localStorage, MEMORY_DIRTY_KEY, "1");
        }
      }
  ...
  useEffect(() => {
    ...
    const sync = async () => {
      ...
      try {
        ...
        let serverState = await fetchServerState(auth.accessToken);
        let serverHistory = sanitizeHistory(serverState.history);
        let serverMemory = sanitizeMemory(serverState.memory);

        /*
         * HISTORY の正本ルール
         *
         * 既にサーバー状態が存在する場合、localStorageの古い履歴を
         * サーバーへマージしない。
         * chat-proxy が user + misaki の会話差分をサーバーへ直接保存するため、
         * 各ブラウザは serverHistory を表示用に受け取るだけにする。
         */
        if (!serverState.exists && localHistory.length > 0) {
          const bootstrap = await fetch("/api/persona/history", {
            ...
            body: JSON.stringify({
              ...
              ...(localMemory.length > 0 ? { memory: localMemory } : {}),
            }),
          });

          if (!bootstrap.ok) {
            throw new Error("Initial conversation sync failed.");
          }

          serverState = await fetchServerState(auth.accessToken);
          serverHistory = sanitizeHistory(serverState.history);
          serverMemory = sanitizeMemory(serverState.memory);
        }

        /* MEMORY は現在の dirty/baseline方式を維持。 */
        const baseline = readMemoryBaseline(auth.userId);
        const dirty = readMemoryDirty();
        const localChanged =
          baseline !== null && !arraysEqual(localMemory, baseline);
        const serverChanged =
          baseline !== null && !arraysEqual(serverMemory, baseline);
        ...
        if (
          ...
          const response = await fetch("/api/persona/history", {
            ...
            body: JSON.stringify({
              history:
                serverHistory.length > 0 ? serverHistory : localHistory,
              memory: localMemory,
            }),
          });
        ...
        /*
         * 重要: ここでマージしない。
         * 各ブラウザはSupabaseの末尾60件をそのまま表示する。
         */
        localStorage.setItem(CHAT_HISTORY_KEY, JSON.stringify(serverHistory));
        writeLocalMemory(serverMemory);
        writeMemoryBaseline(auth.userId, serverMemory);

        if (!dirty || !localChanged || serverChanged) {
          clearMemoryDirty();
        }
