...
export default function ConversationHistorySync() {
  ...
  useEffect(() => {
    ...
    const sync = async () => {
      ...
      const history = sanitizeHistory(readJsonArray(CHAT_HISTORY_KEY));
      const memory = sanitizeMemory(readJsonArray(LONG_MEMORY_KEY));
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(data?.error || "Conversation history sync failed.");
        }
